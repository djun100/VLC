...
public class VideoListAdapter extends ArrayAdapter<Media>
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (v == null || (((ViewHolder)v.getTag()).listmode != mListMode)) {
            ...
            if (!mListMode)
                v = inflater.inflate(R.layout.vlc_video_grid_item, parent, false);
            else
                v = inflater.inflate(R.layout.vlc_video_list_item, parent, false);
        ...
        if (thumbnail == null) {
            // missing thumbnail
            thumbnail = BitmapCache.GetFromResource(v, R.drawable.vlc_thumbnail);
        }
        else if (thumbnail.getWidth() == 1 && thumbnail.getHeight() == 1) {
            // dummy thumbnail
            thumbnail = BitmapCache.GetFromResource(v, R.drawable.vlc_icon);
        }
        //FIXME Warning: the thumbnails are upscaled in the grid view!
        holder.thumbnail.setImageBitmap(thumbnail);

        /* Color state */
        ColorStateList titleColor = v.getResources().getColorStateList(R.color.vlc_list_title);
